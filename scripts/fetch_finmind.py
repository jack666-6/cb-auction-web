...
- 投標結束日前60日股價波動率: 投標結束日往前60個交易日(含當天)母公司現股收盤價的變異係數
...
def fetch_one(cb_code: str, bid_end_date: str, listing_date: str, cbas_prices: dict) -> dict:
    """回傳轉換價、投標結束日現股收盤價、投標結束日前60日股價波動率，
    ...
    if bid_end_date:
        ...
        result["投標結束日前60日股價波動率"] = _price_volatility(vol_closes)
...
def _is_resolved(entry: dict) -> bool:
    ...
    # 就算resolved，不用每次重試
    if "投標結束日前60日股價波動率" not in entry:
        return False
